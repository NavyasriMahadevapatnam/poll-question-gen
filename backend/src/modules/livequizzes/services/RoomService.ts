...
import { ROOM, SCORING, TIME } from '#root/shared/constants/pollConstants.js';
...
export class RoomService {
  ...
  async createRoom(name: string, teacherId: string): Promise<RoomType> {
    const code = Math.random()
      .toString(ROOM.RANDOM_BASE)
      .substring(ROOM.CODE_SUBSTRING_START, ROOM.CODE_SUBSTRING_END)
      .toUpperCase();

    logger.info('Creating new room', { teacherId, roomName: name });
  ...
  async getPollAnalysis(roomCode: string) {
    ...
    if (!room) {
      logger.warn('Room not found for poll analysis', { roomCode });
    ...
    for (const poll of room.polls) {
      for (const answer of poll.answers) {
        ...
        if (answer.answerIndex === poll.correctOptionIndex) {
          ...
          participant.score += SCORING.CORRECT_ANSWER_POINTS;
        } else {
          participant.wrong += 1;
          participant.score += SCORING.WRONG_ANSWER_PENALTY;
        }

        // Calculate time taken for this answer (in seconds)
        const answerTime =
          (answer.answeredAt.getTime() - poll.createdAt.getTime()) / TIME.MS_PER_SECOND;
    ...
    const participants = Array.from(participantsMap.values()).map((p) => {
      ...
      if (p.timeTaken > 0) {
        ...
        const seconds = totalSeconds % TIME.SECONDS_PER_MINUTE;
    ...
    return {
      ...
      duration:
        room.endedAt && room.createdAt
          ? Math.ceil((room.endedAt.getTime() - room.createdAt.getTime()) / TIME.MS_PER_MINUTE) +
            ' mins'
          : 'N/A',
      participants,
      questions,
  ...
  async enrollStudent(userId: string, roomCode: string) {
    ...
    if (isAlreadyEnrolled) {
      logger.info('User already enrolled in course', { userId, roomCode });
      return room;
    }
  ...
  async unEnrollStudent(userId: string, roomCode: string) {
    ...
    const isAlreadyEnrolled = room.students.some((id) => id.equals(userObjectId));
    if (!isAlreadyEnrolled) {
      logger.info('User not enrolled in course', { userId, roomCode });
      return room;
    }
    const updatedRoom = await Room.findOneAndUpdate(
      { roomCode },
