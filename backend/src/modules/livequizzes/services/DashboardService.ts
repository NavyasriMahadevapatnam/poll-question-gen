...
import { SCORING } from '#root/shared/constants/pollConstants.js';
...
export class DashboardService {
  async getStudentDashboardData(studentId: string) {
    ...
    let pollResults: Array<{ name: string; score: number; date: Date }> = [];
    let pollDetails: Array<{ title: string; type: string; timer: string }> = [];
    let activePolls: Array<{ name: string; status: string }> = [];
    let upcomingPolls: Array<unknown> = []; // leave empty if you don't have upcoming logic
    let scoreProgression: Array<{ poll: string; score: number }> = [];
    let roomWiseScores: Array<{
      roomName: string;
      roomCode: string;
      totalPolls: number;
    ...
    for (const room of rooms) {
      ...
      for (const poll of room.polls ?? []) {
        ...
        if (answer) {
          ...
          const score =
            answer.answerIndex === poll.correctOptionIndex ? SCORING.DASHBOARD_CORRECT_POINTS : 0;
          roomScore += score;
          totalScore += score;
    ...
    const participationRate =
      totalPolls > 0 ? `${Math.round((takenPolls / totalPolls) * 100)}%` : '0%';
  ...
  async getTeacherDashboardData(teacherId: string) {
    ...
    for (const room of rooms) {
      ...
      const responseCount =
        room.polls?.reduce((sum, poll) => sum + (poll.answers?.length || 0), 0) || 0;
    ...
    const participationRate =
      totalPolls > 0 ? `${Math.round((totalResponses / totalPolls) * 100)}%` : '0%';
      ...
      faqs: [
        {
          ...
          answer: "Click on 'Create Room' button from the dashboard.",
        },
